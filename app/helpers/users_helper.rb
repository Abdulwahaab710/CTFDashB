...
module UsersHelper
  ...
  def user_status_button(user)
    return link_to('Activate', activate_user_path(user), method: :patch, class: 'btn btn-success') unless user.active
    link_to('Deactivate', deactivate_user_path(user), method: :destroy, class: 'btn btn-danger')
  end
  ...
  def add_admin_link(user)
    link_to(
      'Add admin privileges', add_admin_path(user),
  ...
  def remove_admin_link(user)
    link_to(
      'Remove admin privileges', remove_admin_path(user),
  ...
  def add_organizer_link(user)
    link_to(
      'Add organizer privileges', add_organizer_path(user),
      method: :patch, class: 'btn btn-danger',
  ...
  def remove_organizer_link(user)
    link_to(
      'Remove organizer privileges', remove_organizer_path(user),
      method: :delete, class: 'btn btn-danger',
